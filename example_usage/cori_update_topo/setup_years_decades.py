...
def setup_years(start, end, model, scenario, ensemble):
    topoYear = 2015
    topoInterval = 10 
    topoYearNext = topoYear + topoInterval
    for tIndex, year in enumerate(range(start, end+1)):
        ...
        if year >= topoYearNext:
            topoYear = topoYearNext
            topoYearNext += topoInterval
     
        topoPrefix = 'topo{:04d}/topo'.format(topoYear)
        
        replacements = {'@topoPrefix': '{}'.format(topoPrefix),
