...
do jj=1,my
  if ( TF_draft(ii,jj) .gt. -5.d0 ) then
    melt(ii,jj) = K0(3) * cste * ( TF_draft(ii,jj) + DeltaT_basin_pc50(ii,jj,3) )* abs( mean_TF(basinNumber(ii,jj)+1) + DeltaT_basin_pc50(ii,jj,3) )  
  else
    melt(ii,jj) = 0.d0
  endif
  dT0(basinNumber(ii,jj)+1) = DeltaT_basin_pc50(ii,jj,3)
enddo
enddo
...
DO iter=1,2000
  ...
  do jj=1,my
    if ( TF_draft(ii,jj)+dT(basinNumber(ii,jj)+1) .gt. -5.d0 ) then
      melt(ii,jj) = K0(3) * cste * ( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1) )* abs( mean_TF(basinNumber(ii,jj)+1) + dT(basinNumber(ii,jj)+1) )  
    else
      melt(ii,jj) = 0.d0
    endif
    total( basinNumber(ii,jj)+1 ) = total( basinNumber(ii,jj)+1 ) + melt(ii,jj) * mesh_area(ii,jj) * 1.d-9  ! in Gt/yr
