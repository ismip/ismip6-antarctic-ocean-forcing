...
do jj=1,my
  if ( TF_draft(ii,jj) .gt. -5.d0 ) then
    melt(ii,jj) = K0(3) * cste * max( TF_draft(ii,jj) + DeltaT_basin_pc50(ii,jj,3), 0.d0 )* max( TF_draft(ii,jj) + DeltaT_basin_pc50(ii,jj,3), 0.d0 )
  else
    melt(ii,jj) = 0.d0
  endif
enddo
...
DO iter=1,10000
  ...
  do kbasin=1,Nbasin
    ...
    if ( kbasin .eq. 1 .and. MOD(iter,100) .eq. 0 ) write(*,*) iter, total(kbasin), control(kbasin), dT(kbasin)
  enddo
  ...
  do jj=1,my
    if ( TF_draft(ii,jj)+dT(basinNumber(ii,jj)+1) .gt. -5.d0 ) then
      !melt(ii,jj) = K0(3) * cste * ( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1) )* abs( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1) )
      melt(ii,jj) = K0(3) * cste * max( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1), 0.d0 )* max( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1), 0.d0 )
    else
      melt(ii,jj) = 0.d0
    endif
    total( basinNumber(ii,jj)+1 ) = total( basinNumber(ii,jj)+1 ) + melt(ii,jj) * mesh_area(ii,jj) * 1.d-9  ! in Gt/yr
