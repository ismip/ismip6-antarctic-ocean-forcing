...
file_K  = 'coeff_K0_DeltaT_quadratic_non_local_Rignot_Depoorter_with_TS_uncert.nc'
file_topo  = 'bedmap2_8km.nc'                            ! bedmap2 ice-shelf draft and mask

! output file containing melt rates :
file_melt_out = 'melt_rates_NON_LOCAL_K0_DeltaT.nc'
...
do jj=1,my
  ...
  if ( icemask_shelves(ii,jj) .ge. 5.d-1 .and. ice_draft(ii,jj) .lt. 0.d0 ) then
    ...
    ksup=1
    do kk=2,mz-1
      if ( z(kk) .ge. ice_draft(ii,jj) ) ksup = kk
    enddo
    kinf = ksup + 1
...
do jj=1,my
  if ( TF_draft(ii,jj) .gt. -5.d0 ) then
    melt(ii,jj) = K0(2) * cste * ( TF_draft(ii,jj) + DeltaT_basin_pc50(ii,jj,2) )* abs( mean_TF(basinNumber(ii,jj)+1) + DeltaT_basin_pc50(ii,jj,2) )  
  else
    melt(ii,jj) = 0.d0
  endif
  dT0(basinNumber(ii,jj)+1) = DeltaT_basin_pc50(ii,jj,2)
enddo
enddo
...
DO iter=1,2000
  ...
  do jj=1,my
    if ( TF_draft(ii,jj)+dT(basinNumber(ii,jj)+1) .gt. -5.d0 ) then
      melt(ii,jj) = K0(2) * cste * ( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1) )* abs( mean_TF(basinNumber(ii,jj)+1) + dT(basinNumber(ii,jj)+1) )  
    else
      melt(ii,jj) = 0.d0
    endif
    total( basinNumber(ii,jj)+1 ) = total( basinNumber(ii,jj)+1 ) + melt(ii,jj) * mesh_area(ii,jj) * 1.d-9  ! in Gt/yr
