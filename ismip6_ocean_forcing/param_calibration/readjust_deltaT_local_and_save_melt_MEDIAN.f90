...
do jj=1,my
  if ( TF_draft(ii,jj) .gt. -5.d0 ) then
    melt(ii,jj) = K0(2) * cste * max( TF_draft(ii,jj) + DeltaT_basin_pc50(ii,jj,2), 0.d0 )* max( TF_draft(ii,jj) + DeltaT_basin_pc50(ii,jj,2), 0.d0 )
  else
    melt(ii,jj) = 0.d0
  endif
enddo
...
DO iter=1,10000
  ...
  do jj=1,my
    if ( TF_draft(ii,jj)+dT(basinNumber(ii,jj)+1) .gt. -5.d0 ) then
      !melt(ii,jj) = K0(2) * cste * ( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1) )* abs( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1) )
      melt(ii,jj) = K0(2) * cste * max( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1), 0.d0 )* max( TF_draft(ii,jj) + dT(basinNumber(ii,jj)+1), 0.d0 )
    else
      melt(ii,jj) = 0.d0
    endif
    if ( iter .eq. 10000 .and. MOD(ii,10) .eq. 0 .and. MOD(jj,10) .eq. 0 ) write(*,777) ii, jj, TF_draft(ii,jj), dT(basinNumber(ii,jj)+1), melt(ii,jj)
    total( basinNumber(ii,jj)+1 ) = total( basinNumber(ii,jj)+1 ) + melt(ii,jj) * mesh_area(ii,jj) * 1.d-9  ! in Gt/yr
  enddo
  enddo
...
777 FORMAT(i3.3,i3.3,' ',f7.3,' ', f7.3,' ',f7.3)
